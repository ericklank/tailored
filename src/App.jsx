...
positiveOutcomes and suggestedOutcomes are arrays of objects. All others are arrays of strings except prospectName/prospectCompany/employeeCount which are primitives. No preamble, no markdown, just raw JSON.`,
          messages: [{ role: "user", content: [...fileContents, { type: "text", text: "Analyze all provided materials and return the JSON summary." }] }],
        }),
      });
  ...
  const analyzePickup = async () => {
    ...
    try {
      const fileContents = await buildFileContents();
      const contextLines = buildContextLines();
      const sys = `You are a sales assistant helping a new Teamtailor rep pick up a deal mid-cycle from a colleague who has left the company. Analyze all provided materials (notes, emails, CRM exports, transcripts, etc.) and generate a warm re-introduction email to send to the prospect.${contextLines ? `\n\nContext:\n${contextLines}` : ""}

The email should:
- Open with a warm, professional introduction of the new rep taking over
- Summarize "my current understanding" of where things stand — what problems they have, what was discussed, what Teamtailor features resonated
- Reference any agreed next steps or open items from the notes
- If no clear next steps exist, suggest connecting in the next week to continue the conversation
- Be genuine and human — not overly salesy. The goal is continuity and trust
- Sign off with the rep's name (use Rep Name from context if provided, otherwise leave as [Your name])
- Keep it concise — 150-200 words max

Return ONLY a JSON object with these keys:
- prospectName: string (first name of prospect)
- prospectCompany: string (company name)
- subject: string (email subject line)
- body: string (full email body, use real newlines)
- summary: array of 3-5 short strings summarizing what you found in the notes (shown to the rep only, not sent)
...
No preamble, no markdown, just raw JSON.`;
      const response = await fetch("/api/analyze", {
        ...
        body: JSON.stringify({
          password, system: sys,
          messages: [{ role: "user", content: [...fileContents, { type: "text", text: "Analyze and generate the re-engagement email." }] }],
        }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error?.message || "API error");
            ...
            <>
              <div style={{ display: "flex", gap: 4, marginBottom: 28, background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: 12, padding: 4 }}>
                <button onClick={() => setMode("analyze")} style={{
                  ...
                  background: mode === "analyze" ? "linear-gradient(135deg, #FF2D78, #FF6BA8)" : "transparent",
                  color: mode === "analyze" ? "white" : "var(--text-muted)",
                }}>📞 Analyze a Call</button>
                <button onClick={() => setMode("pickup")} style={{
                  flex: 1, padding: "10px 16px", borderRadius: 9, border: "none", cursor: "pointer",
                  fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 700, transition: "all 0.2s",
                  background: mode === "pickup" ? "linear-gradient(135deg, #6C63FF, #9B8FFF)" : "transparent",
                  color: mode === "pickup" ? "white" : "var(--text-muted)",
                }}>🤝 Pick Up a Deal</button>
              </div>
              ...
              <button
                ...
                onClick={mode === "pickup" ? analyzePickup : analyze}
                style={{ background: mode === "pickup" ? "linear-gradient(135deg, #6C63FF, #9B8FFF)" : undefined }}
              >
            ...
            <div className="results">
              <div className="results-header">
                <div>
                  ...
                  <div style={{ fontSize: 14, color: "var(--text-muted)", marginTop: 4 }}>
                    {pickupEmail.prospectName} · {pickupEmail.prospectCompany}
                  </div>
                </div>
              </div>
              ...
              <div style={{ background: "#F5F3FF", border: "1.5px solid #DDD6FE", borderRadius: 16, padding: 24, marginBottom: 24 }}>
                ...
                <ul className="section-items">
                  {(pickupEmail.summary || []).map((s, i) => (
                    <li className="section-item" key={i}>{s}</li>
                  ))}
                </ul>
              </div>
              ...
              <button className="reset-btn" onClick={() => { setPickupEmail(null); setFiles([]); setMode("analyze"); }}>
                ← Start over
              </button>
            </div>
          )}
